...
export function EmployerDashboardPage({ refreshKey }: { refreshKey: number }) {
  ...
  const confirmedApplications = data.applications.filter((application) => application.status === "confirmed");
  const pendingSettlements = data.applications.filter((application) => application.status === "pending_settlement");

  return (
    <section className="page-stack">
      <header className="page-heading compact-heading employer-heading">
        <div className="eyebrow">招聘方端 · 驿站工作台</div>
        <h1>从报名确认，到工时核定与模拟结算</h1>
      ...
      <section className="metric-grid" aria-label="招聘方工作台概览">
        ...
        <article><span>待核定完工</span><strong>{confirmedApplications.length}</strong><small>仅已确认报名可操作</small></article>
        <article><span>待模拟结算</span><strong>{pendingSettlements.length}</strong><small>唯一结算入口</small></article>
      </section>
      ...
      {data.applications.length === 0 ? (
        <PageFeedback tone="empty" title="当前没有报名" detail="请切换到学生端完成开放岗位报名，再返回这里读取同一条云端记录。" />
      ) : (
        <div className="application-list">
          {data.applications.map((application) => (
            <article className="application-list-item" key={application.id}>
              ...
              <Link className="secondary-button" to={`/employer/applications/${application.id}`}>{application.status === "pending" ? "确认报名" : application.status === "confirmed" ? "核定完工" : "查看记录"}</Link>
            </article>
          ))}
        </div>
      )}
